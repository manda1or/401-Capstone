...
`),await Z()),n&&(console.info(rn),await Z());let u={},f={},p={},h={},g=Date.now(),_=0,[v,y]=pr(G(r));for(let t of v){let n=l(d(t)),r=an[c(t)];if(!gn.has(r)&&!y.has(t))continue;h[r]=t,u[r]||(u[r]={});let a=await mr(n),o=[],s=[];Array.isArray(q.include)&&(o=q.include.filter(e=>typeof e==`string`).flatMap(e=>Cn(e))),Array.isArray(q.exclude)&&(s=q.exclude.filter(e=>typeof e==`string`).flatMap(e=>Cn(e)));let v=ur(o,a?.include??[]),S=ur(s,a?.exclude??[]),C={},w=(0,zt.default)(`npm`,{registry:`https://registry.npmjs.org`})||{},T={npmrc:w,recursive:!0};if(r===`npm`){w[`strict-ssl`]===!1&&(C.rejectUnauthorized=!1);for(let e of[`cert`,`ca`,`key`]){let n=e===`key`?sr:or,r=[];w[e]&&(r=(Array.isArray(w[e])?w[e]:[w[e]]).flatMap(e=>n(e))),w[`${e}file`]&&(r=Array.from(n(m(w[`opt${t}`],`utf8`)))),r.length&&(C[e]=e===`ca`?[...x,...r]:r)}}let E=[];Array.isArray(i)?E=i.filter(e=>typeof e==`string`):`types`in a&&Array.isArray(a.types)?E=a.types:r===`npm`?E=Kt:r===`pypi`?E=[...Yt,...Jt]:r===`go`&&(E=Array.from(Xt));let D={};if(r===`go`)p[r]=b(`go`,[`list`,`-m`,`-f`,`{{if not .Indirect}}{{.Path}}@{{.Version}}{{end}}`,`all`],{stdio:`pipe`,encoding:`utf8`,cwd:n});else try{p[r]=m(t,`utf8`)}catch(e){throw Error(`Unable to open ${t}: ${e.message}`)}try{if(r===`npm`)D=JSON.parse(p[r]);else if(r===`pypi`)D=nt(p[r]);else{D.deps={};for(let e of Mn(p[r])){let[t,n]=e.split(`@`);n&&(D.deps[t]=n)}}}catch(e){throw Error(`Error parsing ${t}: ${e.message}`)}for(let e of E){let t;if(t=r===`npm`||r===`go`?D[e]||{}:Sn(D,e)||{},Array.isArray(t)&&r===`pypi`)for(let{name:n,version:i}of Gt(t))dr(n,r,v,S,e)&&(u[r][`${e} ${n}`]={old:ar(i),oldOriginal:i});else if(typeof t==`string`){let[n,i]=t.split(`@`);u[r][`${e} ${n}`]={old:ar(i),oldOriginal:i}}else for(let[n,i]of Object.entries(t))r!==`go`&&(0,H.validRange)(i)&&dr(n,r,v,S,e)?u[r][`${e} ${n}`]={old:ar(i),oldOriginal:i}:r===`npm`&&dr(n,r,v,S,e)?f[`${e} ${n}`]={old:i}:r===`go`&&(u[r][`${e} ${n}`]={old:ir(i),oldOriginal:i})}if(_+=Object.keys(u[r]).length+Object.keys(f).length,!_)continue;let O;r===`npm`&&(O=W((typeof q.registry==`string`?q.registry:!1)||a.registry||w.registry));let ee=await ce(Object.keys(u[r]).map(e=>async()=>{let[t,n]=e.split(`\0`);if(r===`npm`)return An(n,t,O,C,T,w);if(r===`pypi`)return jn(n,t,C);{let e=await Nn(n,`latest`,C,Array.from(yn)),t=await Pn(n,C);return t&&e&&(0,H.gt)(t.Version.substring(1),e.Version.substring(1))&&(e=t),[e,Xt[0],null,n]}}),{concurrency:e});for(let[e,t,n,i]of ee){if(e?.error)throw Error(e.error);let a=typeof J==`boolean`?J:X(e.name,J),o=typeof dn==`boolean`?dn:X(e.name,dn),s=typeof fn==`boolean`?fn:X(e.name,fn),c;c=pn===!0||X(e.name,pn)?new Set([`patch`]):mn===!0||X(e.name,mn)?new Set([`patch`,`minor`]):new Set([`patch`,`minor`,`major`]);let l=`${t} ${i}`,d=u[r][l].old,f=u[r][l].oldOriginal,p=Qn(e,{usePre:o,useRel:s,useGreatest:a,semvers:c,range:d,mode:r}),m=``;if([`go`,`pypi`].includes(r)&&p?m=p:p&&(m=Jn(d,p,f)),!p||f&&f===m){delete u[r][l];continue}let h=``;r===`npm`&&e.time?.[p]?h=e.time[p]:r===`pypi`&&e.releases?.[p]?.[0]?.upload_time_iso_8601?h=e.releases[p][0].upload_time_iso_8601:r===`go`&&e.Time&&(h=e.Time),u[r][l].new=m,r===`npm`?u[r][l].info=Ln(e?.versions?.[p],n,e.name):r===`pypi`?u[r][l].info=Ln(e,n,e.info.name):u[r][l].info=e?.Origin?.URL??`https://${i}`,h&&(u[r][l].date=h,u[r][l].age=Wt(h,{noAffix:!0}))}if(Object.keys(f).length){let t=(await ce(Object.entries(f).map(([e,t])=>()=>{let n=e.split(`\0`)[1];return rr(e,t,typeof J==`boolean`?J:X(n,J))}),{concurrency:e})).filter(e=>e!==null);for(let e of t){let{key:t,newRange:n,user:i,repo:a,oldRef:o,newRef:s,newDate:c}=e;u[r][t]={old:f[t].old,new:n,oldPrint:en.test(o)?o.substring(0,7):o,newPrint:en.test(s)?s.substring(0,7):s,info:`https://github.com/${i}/${a}`,...c?{age:Wt(c,{noAffix:!0})}:{}}}}let k=q[`min-age`]??a.minAge;if(k){for(let e of Object.keys(u))for(let[t,{date:n}]of Object.entries(u[e]))if(!fr(n,Number(k),g)){delete u[e][t];continue}}}_===0&&Rn(`No dependencies found, nothing to do.`);let S=0;for(let e of Object.keys(u))S+=Object.keys(u[e]).length;S||Rn(`All dependencies are up to date.`);let C=zn(u);if(s){for(let e of Object.keys(u))if(Object.keys(u[e]).length){try{let t=e===`npm`?Kn:qn;Bn(h[e],t(p[e],u[e]))}catch(t){throw Error(`Error writing ${c(h[e])}: ${t.message}`)}console.info(un(`✨ ${c(h[e])} updated`))}}await Z(void 0,C)}try{await hr()}catch(e){await Z(e)}export{fr as canIncludeByDate};